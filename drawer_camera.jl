...
function draw_the_canvas(canvas)
    ...
    camXRZ = rotate_phi_z(az, Z, camXRY)
    camYRZ = rotate_phi_z(az, Z, camYRY)
    camZRZ = rotate_phi_z(az, Z, camZRY)
    ...
    # ----------------------------------------------------------- EXERCISE 3.1
    for i=1:s1
        circle(ctx, 375 + focal_camera(Binv, focus, A[i,:])[1]*100, 500 - focal_camera(Binv, focus, A[i,:])[2]*100, 1)
    ...
    for i=1:s1
        circle(ctx, originX + camCoord2d[1] + focal_camera(Binv, focus, A[i,:])[1]*100, originY + camCoord2d[2] + focal_camera(Binv, focus, A[i,:])[2]*100, 1)
        set_source_rgb(ctx, 0, 0, 0)
        fill(ctx)
    end
